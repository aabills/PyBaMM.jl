...
spm_benchmark = @benchmark solve(prob, QNDF(linsolve=KLUFactorization(),concrete_jac=true))
...
full_sol = solve(prob, QNDF(linsolve=KLUFactorization(),concrete_jac=true))
full_benchmark = @benchmark solve(prob, QNDF(linsolve=KLUFactorization(),concrete_jac=true))
...
ocvr_sol = solve(prob, QNDF(linsolve=KLUFactorization(),concrete_jac=true))
ocvr_benchmark = @benchmark solve(prob, QNDF(linsolve=KLUFactorization(),concrete_jac=true))
...
spme_benchmark = @benchmark solve(prob, QNDF(linsolve=KLUFactorization(),concrete_jac=true))



interpolated_ocvr_sol = hcat(ocvr_sol.(full_sol.t)...)[5:965,:]
interpolated_full_sol = Array(full_sol)[5:965,:]
interpolated_spm_sol = hcat(spm_sol.(full_sol.t)...)[5:965,:]
interpolated_spme_sol = hcat(spme_sol.(full_sol.t)...)[5:965,:]
interpolated_cube_sol = hcat(cubic_ocv_r_sol.(full_sol.t)...)[5:965,:]

ocvr_err = sqrt.(mean((interpolated_ocvr_sol .- interpolated_full_sol).^2,dims=2))
spm_err = sqrt.(mean((interpolated_spm_sol .- interpolated_full_sol).^2,dims=2))
spme_err = sqrt.(mean((interpolated_spme_sol .- interpolated_full_sol).^2,dims=2))
cube_err = sqrt.(mean((interpolated_cube_sol .- interpolated_full_sol).^2,dims=2))

plot(log.(ocvr_err),label="OCV-R",legend=:bottomleft)
plot!(log.(spm_err),label="SPM")
plot!(log.(spme_err),label="SPMe")
plot!(log.(cube_err),label="Cube")
