...
parameter_values["Positive electrode OCP [V]"] = pv_chen["Positive electrode OCP [V]"]

model = pybamm.lithium_ion.DFN(name="DFN", options=options)
netlist = setup_circuit.setup_circuit(Np, Ns, I=curr)
experiment = pybamm.Experiment(repeat(["Charge at 7.5 A until 21 V","Hold at 21 V until 250 mA", "Discharge at 25 A until 12.5 V"],1000))   

pybamm_pack = pack.Pack(model, netlist, functional=functional, thermal=true, operating_mode = experiment, top_bc = "symmetry", right_bc = "symmetry", parameter_values=parameter_values)
...
saved_vars = get_pack_variables(pybamm_pack, integrator.sol, ["Current [A]","Terminal voltage [V]", "Negative electrode potential [V]"])
