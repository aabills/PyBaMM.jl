...
def remove_lines_with(input_string, pattern):
    ...
    for s in string_list:
        ...
        #Special Case: we never want to delete the "let" line
        elif "let" in s:
            my_string = my_string + s + "\n"
    return my_string
...
class PybammJuliaFunction(pybamm.Symbol):
    def __init__(self, children, expr, name, inplace, shape=None):
        self.inplace = inplace
        self.expr = expr
        if expr is None:
            if shape is None:
                raise NotImplementedError(
                    "A PybammJuliaFunction must provide either an expression or an (output) shape"
                )
            self.shape = shape
    ...
    def _convert_tree_to_intermediate(self, symbol):
        if isinstance(symbol, pybamm.NumpyConcatenation):
            ...
            if self._black_box and not self._override_psuedo:
                my_id = symbol.id
                name = symbol.name
                self._intermediate[my_id] = JuliaInput(my_id, name,shape=(1,1))
            else:
